...
def blogview(context, request):
    ...
    for name, blogentry in context.items():
        if IMethodEntry.providedBy(blogentry):
            blogentries.append(
...
class FeedViews(object):
    ...
    def _get_feed_info(self):
        ...
        for name, entry in context.items():
            if IMethodEntry.providedBy(entry):
                updated = entry.pubdate
                info = {'url': resource_url(entry, request),
                        'title': blogentry.title,
                        'body': _getentrybody(entry.format,
                                              entry.entry),
                        'created': updated,
                        'pubdate': updated,
                       }
                _add_updated_strings(updated, info)
                entries.append((updated, info))
                
        entries.sort(key=lambda x: x[0].isoformat())
        entries = [entry[1] for entry in reversed(blogentries)][:15]
        updated = entries and entries[0]['pubdate'] or self._nowtz()
    ...
    def blog_rss(self):
        feed, entries = self._get_feed_info()
